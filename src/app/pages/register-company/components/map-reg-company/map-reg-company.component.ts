...
@Component({
  selector: 'app-map-reg-company',
  templateUrl: './map-reg-company.component.html',
  styleUrls: ['./map-reg-company.component.scss'],
})
export class MapRegCompanyComponent implements OnInit {

  @Input ('modalMap') modalMap = {modal: false, nroAddress: 0, address: '', position: {latitude: 0, longitude: 0}}
  ...
  ngOnInit() {
    this.address = this.modalMap.address;
    this.geo.lat = this.modalMap.position.latitude;
    this.geo.lng = this.modalMap.position.longitude;
    this.getGeolocation();
  }
  ...
  mapModal(cancel: boolean) {
    this.setModalMap.emit({
        modal: false,
        nroAddress: this.modalMap.nroAddress,
